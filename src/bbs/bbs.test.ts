		...
		describe("Blind BBS", async () => {
			...
			describe("signature", () => {
				describe("passes test vectors:", async () => {
					...
					it("valid multi prover committed messages, no signer messages signature", async ({ skip }) => {
						...
						assert(await VerifyBlindSign(PK, signature, header, [...messages, ...committed_messages], 0, secret_prover_blind));
						assert(await VerifyBlindSign(PK, expectedSignature, header, [...messages, ...committed_messages], 0, secret_prover_blind));
					...
					it("valid no prover committed messages, multiple signer messages signature", async ({ skip }) => {
						...
						assert(await VerifyBlindSign(PK, signature, header, [...messages, ...committed_messages], 10, secret_prover_blind));
						assert(await VerifyBlindSign(PK, expectedSignature, header, [...messages, ...committed_messages], 10, secret_prover_blind));
			...
			describe("proof", () => {
				describe("is consistent:", async () => {
					...
					const { BlindBbs: { Commit, BlindSign, BlindProofGen, BlindProofVerify }, params: { curves: { fields: { Fr } } } } = getCipherSuite(
						suiteId,
						{
							// https://www.ietf.org/archive/id/draft-irtf-cfrg-bbs-signatures-08.html#name-mocked-random-scalars
							// with
				...
				describe("passes test vectors:", async () => {
					...
					it("valid all prover committed messages and signer messages revealed proof", async () => {
						...
						const message_disclosures: DisclosureChoice[] = range(messages.length + committed_messages.length).map(() => "DISCLOSE");
					...
					it("valid half prover committed messages and all signer messages revealed proof", async () => {
						...
						const message_disclosures: DisclosureChoice[] = [
							...messages.map<DisclosureChoice>(() => "DISCLOSE"),
							"DISCLOSE", "HIDE", "DISCLOSE", "HIDE", "DISCLOSE",
						];
						const [proof, add_zkp_info] = await BlindProofGen(
							PK,
							signature,
					...
					it("valid half prover committed messages and all signer messages revealed proof", async () => {
						...
						const message_disclosures: DisclosureChoice[] = [
							"DISCLOSE", "HIDE", "DISCLOSE", "HIDE", "DISCLOSE", "HIDE", "DISCLOSE", "HIDE", "DISCLOSE", "HIDE",
					...
					it("valid half prover committed messages and no signer messages revealed proof", async () => {
						...
						const message_disclosures: DisclosureChoice[] = [
							...messages.map<DisclosureChoice>(() => "HIDE"),
							"DISCLOSE", "HIDE", "DISCLOSE", "HIDE", "DISCLOSE",
						];
						const [proof, add_zkp_info] = await BlindProofGen(
							PK,
					...
					it("valid no prover committed messages and no signer messages revealed proof", async () => {
						...
						const message_disclosures: DisclosureChoice[]= range(messages.length + committed_messages.length).map(() => "HIDE");
						const [proof, add_zkp_info] = await BlindProofGen(
							PK,
							signature,
							header,
					...
					it("valid all prover committed messages and signer messages revealed proof", async ({ skip }) => {
						...
						const message_disclosures: DisclosureChoice[] = ["DISCLOSE", "HIDE", "DISCLOSE", "HIDE", "DISCLOSE", "HIDE", "DISCLOSE", "HIDE", "DISCLOSE", "HIDE"]
